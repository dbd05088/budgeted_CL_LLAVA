...
for task_split in task_splits:
    ...
    train_task_data = np.concatenate([train_dict[task] if task != most_num_key else np.random.choice(train_dict[task], size=train_second_most, replace=False) for task in task_split])
    np.random.seed(fixed_seed)
    test_task_data = np.concatenate([test_dict[task]  if task != most_num_key else np.random.choice(test_dict[task], size=test_second_most, replace=False) for task in task_split])
